<main>
  <article>
    ...
    <section>
      ...
      <p>There are also other schemes that you might see in URLs on the Web, for example the "mailto:" scheme which web browsers use to open your email client. These are less important, so just know the scheme is the protocol in which a web browser tries to get a resource or do something.</p>
      ...
      <p>Once we know the scheme, domain, and port that we will be using to request from the web server, next comes the <strong>file path</strong>. In the figure, the file path is "/path/to/file.html". The same way we have files and folders on our personal computers, web servers also have files and folders that they make publicly available on the Web. In this case, the file is "file.html" which lives in folder "/to" which lives in the top-level folder "/path". So a web server could potentially make available thousands of files if they have the infrastructure. These files are often HTML, images like PNGs and JPEGs, MP4 videos, and lots of other file formats that are supported on the Web. So with the file path in the URL, the web server knows which file that we want.</p>
      ...
      <p>Query parameters come after the file path and require a "?" before them. They are essentially key value pairs joined by an "=" that can provide extra data and information to the server, and the server can do what it wants with them. Each time a new query parameter is added, an "&" symbol must separate them. So the query parameter section in the example is "?key1=value1&key2=value2" which essentially says, "key1 has value1 and key2 has value2 for this request".</p>
      ...
      <p>So with this comprehensive structure, URLs allow us to identify any unique resource on the Web which can be any of the many types supported on the Web. And with these unique identifiers, web servers and web browsers are able to easily communicate and retrieve the resources for a specific request. In the next tutorial we will talk about what links are and how they tie into URLs.</p>
    </section>
    <?php
    $keyConcepts = [
